...
void InitLX200Input( void)
{
	...
	strcpy( LX200_Cmd_Str[GetFirmwareIDString], "VF");
...
void ReadLX200Input( void)
{
	...
	/* else LX200 port quiet */
		...
		if( DataReady == True)
		{
			...
			if( TimeDiff <= -OneRev)
				TimeDiff += OneRev;
			/* after 1 second, if nothing further received, and commands do not end with '#', process
			it as unfinished */
			if( TimeDiff > SecToRad)
			{
				...
				if( TimeDiff >= ClockTickToRad && LX200Queue[LX200_Ix] == '#')
				{
					ProcessLX200Cmds();
					DataReady = No;
					/* get ready for next command... */
...
{
	sprintf( StrBuf, "BartelsStepperEQ");
	WriteSerialString( LX200ComPort, StrBuf, strlen( StrBuf));
}
...
/* 012345678901   01234567890
	  _HH:MM:SS# or  _HH:MM.T# ; values may be 1 or 2 chars long */
void LX200_Read_Ra( const int Ix)
{
	...
	DisplayIn( "from LX200", NameBlanks);
...
/* 012345678901    01234567890
	  ...
	  substitude for '*', and values may be 1 or 2 chars long */
...
void LX200_Read_SidT( const int Ix)
{
	...
	NewCurrentSolarTimeRad = CurrentSolarTimeRad - TDiff;
	while( NewCurrentSolarTimeRad < 0)
		NewCurrentSolarTimeRad += OneRev;
	while( NewCurrentSolarTimeRad > OneRev)
		NewCurrentSolarTimeRad -= OneRev;
	/* change system time */
				...
				case 'G':
					...
					switch( LX200Queue[Ix1])
					{
						...
						case 'd':
							...
							LX200_Write_Dec();
							break;
						/* command is either GF or GVF for get FIELD radius of the field operation */
						case 'F':
						case 'V':
							IncrIx++;
