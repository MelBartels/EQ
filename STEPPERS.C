...
Module algorithms:
	...
	that the choice of 4 or 5 phases for  has no effect on the parallel port outputs for the field
	rotation motor.
...
{
	disable();
	/* restore interrupt vector set in InitSteppers() */
...
void CreateHsArrays( void)
{
	...
	HsOut = (struct AZByte*) malloc( (MaxHsIx + 1) * sizeof( struct AZByte));
	if( HsOut == NULL)
...
void CreateMsArrays( void)
{
   ...
   and winding # (from 0 to MotorWinding-1) = (MsIx/MaxPWM)/Ms */
	...
	MsPWM = (struct AZByte*) malloc( (MaxMsIx + 1 + MaxPWM) * sizeof( struct AZByte));
	if( MsPWM == NULL)
...
void InitMsArrays( void)
{
	...
	for( Winding = 1; Winding <= MotorWindings; Winding++)
	{
		...
		for( MicroStep = 0; MicroStep < Ms; MicroStep++)
		{
			...
			for( IxB = 0; IxB < loopc; IxB++)
				MsPWM[IxA+IxB].Z += WindingA << MotorWindings;
			...
			for( IxB = 0; IxB < loopc; IxB++)
				MsPWM[IxA+IxB].Z += WindingB << MotorWindings;
...
/* MoveHs_f_ptr is set to MoveHs() or other functions in InitSteppers(); MoveHs_f_ptr is called by:
...
void MoveHs( void)
{
	...
	for interrupt timer timing of halfsteps, LX200 port remains enabled */
	if( HsTimerFlag)
	{
	...
	while( !AbortState && (Steps.A || Steps.Z))
	{
		...
		/* do smaller # of steps 1st, unless a motor has 0 steps to do: flags set for Hold() depend
		on this mvmt algorithm */
		if( Steps.A == 0)
			MvmtHs = Steps.Z;
		else if( Steps.Z == 0)
			MvmtHs = Steps.A;
...
void MoveHsUsingIRQTimer( void)
{
	...
	disable();
	/* ClockVect = getvect( Timer_Int); set once in InitSteppers() */
	...
	*/
	while( SlewState != SlewDone)
	{
		...
		if( AutoGEMFlip && !MoveToCurrentRaDecFlag)
		{
			if( (GEMflippedFlag && Current.Az > HalfRev+AutoGEMFlipFuzzRad && Current.Az < OneRev && Dir.Z == CW)
			|| (!GEMflippedFlag && Current.Az > HalfRev && Current.Az < OneRev-AutoGEMFlipFuzzRad && Dir.Z == CCW))
			{
				disable();
				AbortState = GEMflipAbort;
				enable();
			}
		}
